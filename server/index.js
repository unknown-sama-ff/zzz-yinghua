  ...
  const body = {
    ...
    messages: [{
      ...
      content: [
        ...
        { type: 'text', text: 'Look at this character image and locate their face. It may be human, robot, animal, or any creature.\n\nReturn a JSON object with these fields (all values are 0-1 fractions of IMAGE dimensions, top-left origin):\n- faceTop: top edge of face in IMAGE coordinates (smallest Y value — the highest point of the head/ears in the image)\n- faceBottom: bottom edge of face in IMAGE coordinates (largest Y value — the lowest point of chin/muzzle in the image)\n- faceLeft: leftmost edge of facial features\n- faceRight: rightmost edge of facial features\n- bodyAxisAngle: the angle in degrees of the character\'s primary body axis as it appears in the IMAGE. Measure the slope of the spine/torso line as drawn on the image. Convention: 0° = perfectly horizontal body, 90° = perfectly vertical/upright body. SIGN: positive = the character\'s head end is higher (smaller Y) than the feet end on the RIGHT side of the image; negative = the character\'s head end is on the LEFT and is LOWER than where the body extends to the right, OR the head is on the right and lower than the body extends left. In plain terms: look at which end of the body is higher in the image — if the head is upper-LEFT and feet are lower-RIGHT, the angle is NEGATIVE (e.g. -10° to -20°). If the head is upper-RIGHT and feet are lower-LEFT, the angle is POSITIVE. If upright (head at top, feet at bottom), angle is near 90°. Range: -60 to +60.\n\nExamples:\n- Upright standing character: bodyAxisAngle ~85\n- Leaning diagonally (head upper-right): bodyAxisAngle ~35\n- Lying down, head on LEFT side of image, body extends to lower-RIGHT: bodyAxisAngle ~-12\n- Lying down, head on RIGHT side of image, body extends to lower-LEFT: bodyAxisAngle ~12\n\nCRITICAL: The face (especially the eyes) is the absolute protected anchor zone. faceTop/faceBottom must tightly bracket the face so the cut lines pass through chin/forehead areas, NEVER cutting across the eyes.\n\nUpright example: {"faceTop":0.15,"faceBottom":0.40,"faceLeft":0.30,"faceRight":0.55,"bodyAxisAngle":85}\nLying (head left) example: {"faceTop":0.25,"faceBottom":0.62,"faceLeft":0.05,"faceRight":0.38,"bodyAxisAngle":-12}\n\nReply with ONLY the JSON object. No markdown, no explanation.' },
      ],
    }],
    max_tokens: 200,
