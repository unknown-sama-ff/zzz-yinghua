...
app.post('/api/detect-face', async (req, res) => {
  ...
  const body = {
    ...
    messages: [{
      ...
      content: [
        ...
        { type: 'text', text: 'Look at this character image and locate their face. It may be human, robot, animal, or any creature.\n\nReturn a JSON object with exactly these fields:\n\nfaceTop, faceBottom, faceLeft, faceRight\n  All are 0-1 fractions of IMAGE pixel dimensions (origin top-left).\n  faceTop = smallest Y (topmost pixel of head/ears in the image).\n  faceBottom = largest Y (bottommost pixel of chin/muzzle in the image).\n  These are ALWAYS in image Y-axis — even if the character is upside-down or sideways.\n\nbodyAxisAngle\n  Use this simple 2-point method — it is robust even for curled, foreshortened, or self-obscured poses:\n  STEP 1: Find the face center point (average of faceTop/faceBottom/faceLeft/faceRight).\n  STEP 2: Find the "body exit point" — the single point on the image\'s outer edge (top/bottom/left/right border) where the character\'s torso, hips, or legs mass exits or points toward, i.e. the general direction the body extends AWAY from the face, ignoring raised arms/hands/hair that stick out incidentally. For a curled-up or knees-bent pose, this is where the knees/hips/lower body sit or lead toward, NOT where a raised hand or peace-sign gesture points.\n  STEP 3: Draw a line from the face center to the body exit point. Measure its angle.\n\n  Do NOT use shoulder line, hip line, or arm/limb direction alone — those are unreliable when limbs are bent, crossed, or occlude the torso. The face-center → body-mass-direction line is what matters.\n\n  ANGLE CONVENTION (image coordinates, Y increases downward):\n  • 90°  = body extends straight DOWN from the face (normal upright stance, head at top)\n  • -90° = body extends straight UP from the face (upside-down)\n  •   0° = body extends horizontally, to the RIGHT of the face\n  • 180°/-180° = body extends horizontally, to the LEFT of the face\n  • Positive angle: body mass is toward the lower-right or right of the face.\n  • Negative angle: body mass is toward the lower-left or left of the face.\n\n  POSE REFERENCE:\n  • Upright / standing / sitting upright, body straight down     →  ~85° to 90°\n  • Strong lean, body toward lower-right                          →  +30° to +55°\n  • Strong lean, body toward lower-left                           →  -30° to -55°\n  • Reclining/lying, face toward camera, body extends to the RIGHT →  +5° to +30°\n  • Reclining/lying, face toward camera, body extends to the LEFT  →  -5° to -30°\n  • Curled up / knees drawn toward chest, body still mostly below/beside face → judge by where the bulk of hips+legs sits relative to the face, typically ±10° to ±35°\n  • Crouching / squatting (body still mostly below face)          →  ~70° to 80°\n  • Jumping / mid-air, angled body                                →  measure face-to-body-mass line directly\n  • Upside-down (body extends upward from face)                  →  ~-85° to -90°\n  Valid range: -90 to +90 (fold left/right-horizontal cases into this range by sign as shown above).\n\ncompactPose (boolean)\n  Set to true ONLY when the body has NO single dominant direction — a tightly curled/balled-up pose (fetal position, hugging knees to chest), a symmetric mid-air pose with limbs splayed in multiple directions (starfish jump), or a close framing where almost the whole frame is face/head with little visible body to indicate a direction. In these cases bodyAxisAngle is unreliable/arbitrary — still provide your best-guess bodyAxisAngle, but set compactPose to true so the caller uses a different cut strategy.\n  Set to false for all normal poses (standing, leaning, lying, crouching with a clear knee/hip direction, reaching, jumping with a clear diagonal body line).\n\nCRITICAL: faceTop/faceBottom must tightly wrap the face (eyes are the protected center). The cut lines pass near chin and forehead — NEVER through the eyes.\n\nExamples:\n{"faceTop":0.12,"faceBottom":0.38,"faceLeft":0.32,"faceRight":0.62,"bodyAxisAngle":85,"compactPose":false}\n{"faceTop":0.20,"faceBottom":0.52,"faceLeft":0.45,"faceRight":0.80,"bodyAxisAngle":40,"compactPose":false}\n{"faceTop":0.28,"faceBottom":0.60,"faceLeft":0.05,"faceRight":0.40,"bodyAxisAngle":-13,"compactPose":false}\n{"faceTop":0.30,"faceBottom":0.58,"faceLeft":0.55,"faceRight":0.90,"bodyAxisAngle":13,"compactPose":false}\n{"faceTop":0.55,"faceBottom":0.80,"faceLeft":0.30,"faceRight":0.60,"bodyAxisAngle":-85,"compactPose":false}\n{"faceTop":0.15,"faceBottom":0.45,"faceLeft":0.30,"faceRight":0.65,"bodyAxisAngle":30,"compactPose":true}\n\nReply with ONLY the JSON object. No markdown, no explanation.' },
      ],
    }],
    max_tokens: 200,
  ...
  try {
    ...
    const compactPose = Boolean(coords.compactPose);
    console.log(`[detect-face] faceTop=${faceTop} faceBottom=${faceBottom} faceLeft=${faceLeft} faceRight=${faceRight} bodyAxisAngle=${bodyAxisAngle} compactPose=${compactPose}`);
    return res.json({ ok: true, faceTop, faceBottom, faceLeft, faceRight, bodyAxisAngle, compactPose });
  } catch (err) {
