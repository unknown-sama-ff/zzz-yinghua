  ...
  const body = {
    ...
    messages: [{
      ...
      content: [
        ...
        { type: 'text', text: 'Look at this character image and locate their face. It may be human, robot, animal, or any creature.\n\nReturn a JSON object with these fields (all values are 0-1 fractions):\n- faceTop: top edge of face/head (above eyes/brow for human, above visor for robot, above eyes for animal)\n- faceBottom: bottom edge of face (bottom of chin for human, bottom of faceplate for robot, bottom of muzzle for animal)\n- faceLeft: leftmost edge of facial features\n- faceRight: rightmost edge of facial features\n- bodyAxisAngle: the angle in degrees of the character\'s primary body motion axis. This is the dominant directional line of the pose — the spine, torso, or core limb extension. The cut lines in this visual system follow the character\'s own body movement, not a fixed template: a lying character cuts diagonally along the body\'s natural flow; a leaning-forward character cuts downward along the spine; a reaching character cuts along the extended limb. The cut originates from the character\'s posture, expressing a "breaking through the frame" dynamic. 0° = horizontal, 90° = vertical, positive = right side higher. Examples: leaning diagonally ~35°, arm stretched upward ~60°, side-lying ~5°, upright ~85°. Range -60 to +60.\n\nCRITICAL: The face (especially the eyes) is the absolute protected anchor zone. The solid band must center on the face — cut lines should sweep past the chin, behind the head, or along the shoulders, NEVER cutting across the eyes or facial features. The face remains the single unbreakable focal point in this visual language.\n\nExample: {"faceTop":0.15,"faceBottom":0.40,"faceLeft":0.30,"faceRight":0.55,"bodyAxisAngle":85}\n\nReply with ONLY the JSON object. No markdown, no explanation.' },
      ],
    }],
    max_tokens: 200,
