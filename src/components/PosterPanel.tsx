...
const POSTER_VARIANTS: PosterVariant[] = [
  {
    ...
    label: '海报版',
    template:
      '严格参考并保留上传图片中角色的面部特征、发型发色、服装细节与配色，确保为同一角色、不改变身份特征；在此前提下进行如下风格化：绝区零影画风格，角色动态动作姿势，重墨黑白线稿与大块剪影，高对比明暗，仅以单一「{DOMINANT_COLOR}」色作点缀强调，辅以「{ACCENT_COLOR}」，图片顶部左侧超大做旧印刷体「{NAME_TOP}」，图片底部右侧超大做旧印刷体「{NAME_BOTTOM}」，角色主体站在画面中央人物脸部清晰不被文字遮挡，底部有一行小字星级副标题信息，斜切几何色块，胶带/贴纸做旧质感，噪点与扫描线，无水印。',
  },
  {
    id: 'author',
    label: '自由版',
...
function fillPoster(template: string, name: string, dominant: string, accent: string, textTop?: string, textTopBright?: string, textBottom?: string): string {
  ...
  return template
    ...
    .replaceAll('{TEXT_TOP}', textTop ?? '#1a1a2e')
