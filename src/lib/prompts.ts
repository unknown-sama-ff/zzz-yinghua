...
export const FIDELITY_PREFIX =
  '严格参考并保留上传图片中角色的面部特征、发型发色、服装细节与配色，确保为同一角色、不改变身份特征；主角色图与原始立绘是唯一角色身份参考，生成的必须是上传图片中的角色本人，严禁自行设计或创造新角色，所有面部特征、五官比例、发型发色、体型、服装细节必须与三视图的角色完全一致；严禁生成参考样张中的任何角色、脸型、五官、发型、发色、身体、服装、配饰或姿势——样张仅可用于借鉴构图方向、色彩氛围和文字排版风格，任何情况下都不得复制或借用样张中的人物形象；如与主角色冲突一律以主角色身份、服饰与面部为准；如果参考图中包含额外道具/武器/装饰元素，仅将其作为可加入角色动作设计的附加元素参考；不得改变角色身份、发型、面部和主体服饰风格；画面必须为横向构图（landscape，宽大于高，3:2 横幅），不需要展示角色全身，突出角色特点与个性，但角色下胸部以上必须完整呈现于画面中；角色做出动作后人物主体约占画面80%~95%；三张影画必须以原三视图的画风为准，角色动作、位置、构图与文字位置必须保持一致（三命和六命严格对齐零命），但各自的色彩与渲染风格由下方风格化指令决定；最终画面只保留角色主体与正常比例英文名字文字，不要任何额外贴纸、徽标、条码、说明字、小标签或背景装饰。在此前提下进行如下风格化：';
