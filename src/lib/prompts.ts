...
export const FIDELITY_PREFIX =
  '严格参考并保留上传图片中角色的面部特征、发型发色、服装细节与配色，确保为同一角色、不改变身份特征；主角色图与原始立绘是唯一角色身份参考，生成的必须是上传图片中的角色本人，严禁自行设计或创造新角色，所有面部特征、五官比例、发型发色、体型、服装细节必须与三视图的角色完全一致；严禁生成参考样张中的任何角色、脸型、五官、发型、发色、身体、服装、配饰或姿势——样张仅可用于借鉴构图方向、色彩氛围和文字排版风格，任何情况下都不得复制或借用样张中的人物形象；如与主角色冲突一律以主角色身份、服饰与面部为准；如果参考图中包含额外道具/武器/装饰元素，仅将其作为可加入角色动作设计的附加元素参考；不得改变角色身份、发型、面部和主体服饰风格；画面必须为横向构图（landscape，宽大于高，3:2 横幅），不需要展示角色全身，突出角色特点与个性，但角色腰部以上必须完整呈现于画面中；角色做出动作后人物主体约占画面70%-85%，允许为角落大字与留白预留空间；三张影画（零命/三命/六命）必须与原三视图的角色美术风格完全一致，不可改变画风；最终画面只保留角色主体与正常比例英文名字文字，不要任何额外贴纸、徽标、条码、说明字、小标签或背景装饰。在此前提下进行如下风格化：';
...
export const TEXT_INSTRUCTION =
  '做旧印刷体英文「{NAME_TOP}」和「{NAME_BOTTOM}」分别放置于两个角落，形成对角或同侧配对（左上+左下 或 左上+右下 或 右上+左下 或 右上+右下 四选一），采用超大号字体，文字允许与角色局部叠压但不得遮挡脸部主体，文字不透明度100%、完全实心，不可透过文字看到后方角色';
...
export const YINGHUA_STYLES: YinghuaStyle[] = [
  {
    ...
    promptTemplate:
      '参考图包含原始角色立绘与风格样张：原始角色立绘用于身份、面部、发型、服饰与配色参考；样张仅用于绝区零影画的整体构图方向、色彩关系与角落文字排版参考，严禁借用样张中的人物外形和名字。绝区零官方Mindscape Cinema影画风格，暗色剪影印刷美学（Dark Silhouette Print Style）：基于原始角色立绘生成基准图{CHARACTER_TRAITS}；人物构图允许明显偏左或偏右，严禁正面平视、证件照式大头、对称站像；允许头顶、肩部、手臂、发尾等局部出框，人物主体大面积裁切，优先冲击感而不是完整呈现；角色主体和文字共占约占画面80%~85%；动作设计具有艺术感、展现角色个性特点；画风与原三视图保持完全一致。文字排版为画面强制核心要素：采用超大号且必须水平排版的做旧印刷体英文{NAME_TOP}和{NAME_BOTTOM}，以双角配对方式放置，严禁旋转、斜切、弧排或透视变形；{NAME_TOP}文字组为三者中最大最宽的文字层，宽度接近画面左右边缘，高度约达画面的三分之一；{NAME_BOTTOM}文字组明显小于{NAME_TOP}，约为{NAME_TOP}的六到七成大小；{NAME_TOP}位于角色后方作为背景层，{NAME_BOTTOM}压于角色前方作为前景层；文字必须与角色产生重叠和前后遮挡关系，严禁将文字缩到空白角落或边缘空旷处；文字可压住肩、发、手臂等局部，但不得遮挡脸部主体；{NAME_TOP}文字颜色使用比背景更深一阶的近黑版印刷色，保留轻微磨损与印刷质感；{NAME_BOTTOM}文字颜色使用比{DOMINANT_COLOR}亮一阶的颜色，保留轻微磨损与印刷质感；文字不透明度100%、完全实心，不可透过文字看到后方角色。角色主体以接近纯黑的深黑块面为主（不是背景色），形成平面压印效果；主体的明暗仅由配色占比决定，严禁任何方向性光源、模拟打光、阴影投射，同一材质不得出现左亮右暗的分裂明暗；只保留极弱的明暗差异与少量边缘轮廓变化来提示结构。角色内部色彩控制：脸部与身体肤色保持接近纯黑的深黑色，在黑色基础上叠加薄薄一层约5%的{DOMINANT_COLOR}或{ACCENT_COLOR}作为肤色罩染，严禁脸部发亮；毛发、服饰、挂饰、配件、发饰等所有非皮肤部位必须统一使用相对更亮的颜色，不得出现只有头发亮或只有衣服亮等局部亮的情况，且不得比肌肤更黑，颜色亮度可上下浮动30%来表达层次感；主体与背景必须明确分离，只允许极少量边缘被背景色轻微染边，大面积块面绝不能变成背景色本身。服装本体、花纹、布料印花、袖口纹样、腰带图案压在深暗色域内，花纹仅以低对比度暗纹或压印感存在，不得形成可读亮花纹。高光只限极少量眼部微光、极细外轮廓描线和少量发丝边缘。背景使用高饱和亮色纯色平涂，优先使用{DOMINANT_COLOR}或{ACCENT_COLOR}作为大面积底色。角色动态：{ACTION_POSE}；整体效果：高对比、强设计感、深黑压印主体、主题色背景、水平超大角落压版文字、强烈留白与斜向冲击感。',
  },
  {
    id: 2,
    label: '三命 · 暗色背景 / 亮色主角',
  ...
  },
  {
    ...
    promptTemplate:
      '你将收到三张独立参考图：第一张为零命成图，仅用于锁定角色姿势、构图比例、取景范围与文字位置；第二张为原始角色立绘，作为角色身份、面部、发型、服饰、配色的唯一权威来源；第三张为风格样张，仅用于借鉴影画的色彩关系、排版风格与印刷质感。你必须在第二张原始立绘的角色身份基础上，按照第一张零命的构图与文字布局，结合第三张的风格参考，生成最终图像{CHARACTER_TRAITS}；角色充满画面（与零命一致）；画风与原三视图保持完全一致；色彩与上色——全彩高饱和赛璐珞：角色进行完整全彩高饱和赛璐珞上色，色彩鲜艳饱满、明亮夺目，严禁暗淡、灰蒙、去饱和或黑白灰处理；真实自然的肤色质感（暖调蜜桃肤色，带有自然红晕与高光）；以「{DOMINANT_COLOR}」为主色调贯穿角色服装、头发、配饰，配合「{ACCENT_COLOR}」作为辉光、高光边缘、点缀色与氛围色提高整体色彩冲击力；肌肤展现（六命专属核心要求，优先级最高）：在保持角色姿势和面部身份不变的前提下，原三视图的服装必须进行大幅删减以大幅增加肌肤露出。必须执行以下至少4项处理：去除外套/披风、去除外层遮挡/护甲、大幅缩短裙摆至大腿以上、大幅缩短袖长至肩部附近、降低领口露出锁骨与肩颈、开放整个背部、开放腰腹、减少腿部布料覆盖至短裤/热裤程度。保留的服装必须保持原三视图的版型与材质，删减不是修改服装本身的设计。背景——纯白无杂质：纯白色（#FFFFFF）铺满整个画面背景区域，无任何渐变、噪点、纹理、阴影或暗角；文字处理：采用超大号但正常比例、且必须水平排版的做旧印刷体英文{NAME_TOP}和{NAME_BOTTOM}，以双角配对方式放置（左上+左下/左上+右下/右上+左下/右上+右下四选一），baseline 与画面上下边平行，严禁旋转、斜切、弧排、透视变形以及横向/纵向拉伸压缩；{NAME_TOP}文字组为三者中最大、最宽的文字层，宽度应接近画面左右边缘，约占画面高度的35%~40%；{NAME_BOTTOM}文字组明显小于{NAME_TOP}，约占{NAME_TOP}宽度的77%~98%、高度的70%~84%。两者均不得压缩或拉伸字形，可通过适度扩大字符间隙来适配版面宽度；优先单侧或对角大字压版，{NAME_TOP}位于角色后方作为背景层，{NAME_BOTTOM}压于角色前方作为前景层，贴近边缘、接近满幅块面，但不是整条上/下边缘铺满；可大面积占据角落块面，允许未放字的其余角落自然留白；文字可明显压住肩、发、手臂等主体局部，但不得遮挡脸部主体；{NAME_TOP}文字颜色使用比{DOMINANT_COLOR}亮一阶的高饱和颜色；{NAME_BOTTOM}文字颜色使用比{ACCENT_COLOR}亮一阶的高饱和颜色；角色动态：{ACTION_POSE}；质感与细节：强烈明暗对比，做旧噪点，斜切几何色块，工业贴纸UI质感，网点纸/半调纹理（halftone dot pattern），印刷做旧质感，扫描线；角色主体脸部清晰，细节高精度，整体呈现绝区零官方Mindscape Cinema影画的明亮全彩印刷品美学。',
  },
];
