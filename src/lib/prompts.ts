...
export const YINGHUA_STYLES: YinghuaStyle[] = [
  {
    ...
    promptTemplate:
      '参考图包含原始角色立绘与零命风格样张：原始角色立绘用于身份、面部、发型、服饰与配色参考；样张仅用于绝区零影画的整体构图方向、色彩关系与角落文字排版参考，严禁借用样张中的人物外形。绝区零官方Mindscape Cinema影画风格，暗色剪影印刷美学（Dark Silhouette Print Style）：基于原始角色立绘生成零命基准图{CHARACTER_TRAITS}；角色构图为强俯视或侧上俯拍的强烈斜向动态半身近景，头部朝画面角落倾斜，人物构图允许明显偏左或偏右，严禁正面平视、证件照式大头、对称站像；允许头顶、肩部、手臂、发尾等局部出框，人物主体大面积裁切，优先冲击感而不是完整呈现；角色主体约占画面65%~78%，必须留出一整块约30%~40%的纯色留白，用于海报压版文字；动作设计具有艺术感、展现角色个性特点；画风与原三视图保持完全一致。角色主体应以接近纯黑的深黑块面为主，形成明确的黑版主体（不是背景色）；外轮廓必须硬朗清晰、边缘分明、黑白块面关系明确。主体整体更接近平面压印效果，而不是明显三维光影建模；只保留极弱的明暗差异与少量边缘轮廓变化，用于提示头发、服装结构、布料褶皱、配饰轮廓和手部姿态。角色内部色彩控制：接近纯黑的深黑块面约70%~80%，带极弱冷紫/冷洋红倾向的深暗色约15%~25%；主体与背景必须有明显分离，不允许主体与背景同色化。服装本体、花纹、布料印花、袖口纹样、腰带图案都应压在深暗色域内，不应出现醒目亮色；花纹仅允许以低对比度暗纹、压印感或几乎融入布料的深色纹样存在，不得形成高可读性亮花纹。高光只限极少量眼部微光、极细外轮廓描线和少量发丝边缘，不允许大面积衣料高光、亮纹样、亮带子、被提亮的发饰，或明显发光的服饰花纹。主体应更深、更压，但必须与高饱和背景形成明确分离；只允许极少量边缘被背景色轻微染边，主体大面积块面绝不能变成背景色本身。背景必须是高饱和亮色纯色平涂，优先使用偏粉、偏洋红、偏紫红的「{DOMINANT_COLOR}」或「{ACCENT_COLOR}」作为大面积底色，整体更像压版海报底色，而不是干净空背景；不要贴纸、条码、标签、图标、网点纸、胶带、扫描线或任何工业装饰。文字处理：采用超大号但正常比例、且必须水平排版的做旧印刷体英文「{NAME_TOP}」和「{NAME_BOTTOM}」，以双角配对方式放置（左上+左下/左上+右下/右上+左下/右上+右下四选一），baseline 与画面上下边平行，严禁旋转、斜切、弧排、透视变形以及横向/纵向拉伸压缩；至少一组文字应跨越画面高或宽的40%~55%，形成明显的海报版式层；优先单侧或对角大字压版，贴近边缘、接近满幅块面，但不是整条上/下边缘铺满；可大面积占据角落块面，允许未放字的其余角落自然留白；文字可明显压住肩、发、手臂等主体局部，但不得遮挡脸部主体；文字颜色使用比背景更深一阶的近黑版印刷色，而不是亮粉色文字块，保留轻微磨损与印刷质感，使文字更像压印在海报上的黑版字层。角色动态：{ACTION_POSE}；整体效果：高对比、强设计感、深黑压印主体、亮粉/洋红背景、水平超大角落压版文字、强烈留白与斜向冲击感，更接近参考图2与影画样式1。',
  },
  {
    id: 2,
    label: '三命 · 暗色背景 / 亮色主角',
