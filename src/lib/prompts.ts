...
export const YINGHUA_STYLES: YinghuaStyle[] = [
  {
    ...
    promptTemplate:
      '参考图包含原始角色立绘与零命风格样张：原始角色立绘用于身份、面部、发型、服饰与配色参考；样张仅用于绝区零影画的整体构图方向、色彩关系与角落文字排版参考，严禁借用样张中的人物外形。绝区零官方Mindscape Cinema影画风格，暗色剪影印刷美学（Dark Silhouette Print Style）：基于原始角色立绘生成零命基准图{CHARACTER_TRAITS}；角色构图为强烈斜向动态半身近景，人物构图允许明显偏左或偏右，避免正面居中站像；允许头顶、肩部、手臂、发尾等局部出框，人物主体大面积裁切，优先冲击感而不是完整呈现；角色充满画面（占85%~95%），优先大面积裁切而非完整呈现；动作设计具有艺术感、展现角色个性特点；画风与原三视图保持完全一致。角色整体以深黑紫、深炭黑、深梅紫等极深色调为主，形成强烈剪影感，但不是纯黑实心块；必须保留角色头发层次、服装结构、布料褶皱、配饰轮廓、手部姿态与面部大轮廓的可辨识度，通过极深色内部的轻微明暗变化与边缘高光来体现细节。角色内部色彩控制：深黑色约55%~65%，带紫调/红调的深暗色约25%~35%，极少量「{DOMINANT_COLOR}」或「{ACCENT_COLOR}」只用于眼部微光、发丝受光、饰品高光和边缘描线，绝不能变成普通全彩插画。主体应更深、更压，减少明亮块面，让深暗主体在高饱和背景上形成更强压迫感。背景必须是高饱和亮色纯色平涂，优先使用偏粉、偏洋红、偏紫红的「{DOMINANT_COLOR}」或「{ACCENT_COLOR}」作为大面积底色，整体更像压版海报底色，而不是干净空背景；不要贴纸、条码、标签、图标、网点纸、胶带、扫描线或任何工业装饰。文字处理：采用超大号但正常比例的做旧印刷体英文「{NAME_TOP}」和「{NAME_BOTTOM}」，以双角配对方式放置（左上+左下/左上+右下/右上+左下/右上+右下四选一），不拉伸不压缩，不把整条边缘铺满；优先单侧或对角大字压版，贴近边缘、接近满幅块面，但不是整条上/下边缘铺满；可大面积占据角落块面，允许未放字的其余角落自然留白；文字可明显压住主体局部，但不得遮挡脸部主体；文字颜色使用与背景同色系但更深或更亮一级的印刷色，保留轻微磨损与印刷质感。角色动态：{ACTION_POSE}；整体效果：高对比、强设计感、深暗主体、亮粉/洋红背景、超大角落压版文字、局部细节可辨，更接近参考图2的斜向冲击感。',
  },
  {
    id: 2,
    label: '三命 · 暗色背景 / 亮色主角',
