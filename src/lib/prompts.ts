...
export const TEXT_INSTRUCTION =
  '做旧印刷体英文「{NAME_TOP}」和「{NAME_BOTTOM}」分别放置于两个角落，形成对角或同侧配对（左上+左下 或 左上+右下 或 右上+左下 或 右上+右下 四选一），采用超大号但正常比例的字体，不允许横向或纵向拉伸压缩；文字可大面积占据角落块面，但不是整条边缘被铺满，允许未放字的另外两个角自然留白；文字允许与角色局部叠压但不得遮挡脸部主体，文字不透明度100%、完全实心，不可透过文字看到后方角色';
