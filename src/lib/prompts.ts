...
export const YINGHUA_STYLES: YinghuaStyle[] = [
  ...
  },
  {
    ...
    promptTemplate:
      '这是零命成图（左侧），你需要对其做风格变换。右侧小图为原始角色立绘，仅用于提取服装配色——严禁参考其姿势、构图或人物位置。绝对禁止移动、缩放、旋转、裁切或重新生成左侧画面中的任何元素。唯一可改：色彩、渲染、材质。角色身体的轮廓、位置、大小、角度、文字的字形、字距、行距、与边缘的距离、被裁切的程度，全部锁定为输入图片中的样子，不可有任何像素级偏移。具体渲染风格：将零命成图的暗色剪影风格变换为极端去饱和灰度主调 + 高饱和点缀色。你收到的参考板左侧为零命成图（仅用于编辑基础），中间为原始角色立绘（仅用于提取服装配色、花纹、配饰细节，严禁复制其角色姿势或构图），右侧为风格样张（仅用于影画色彩排版参考）。再次强调：除色彩外，画面中的一切元素必须与输入图片（左侧零命成图）逐像素完全一致。画风与输入图片（零命成图）保持完全一致；色彩处理——极端去饱和灰度主调 + 高饱和点缀色：角色整体进行彻底去饱和处理，仅保留极少数指定点缀色；注意文字颜色不受去饱和影响，必须保留高饱和色彩；服装本体（上衣、裙子、裤子、外套等）必须与参考板中间的原始立绘完全一致，不得自行修改或增减任何服装部件，仅可改变其色彩，统一为黑白灰三色，建立从近白（亮度230-255）到近黑（亮度10-30）的完整灰阶，通过强烈的块面光影明暗（高光→中间调→阴影）与清晰外轮廓线稿体现立体层次、布料褶皱与发丝质感；角色肌肤为浅灰至中灰色调（亮度100-180，完全去饱和）；眼睛虹膜保留「{DOMINANT_COLOR}」（80%色彩饱和度），并带有明显的高光反射点；指定点缀部位（包括但不限于：披风、指甲、手套、唇舌、心形/星形等标志性配饰、发饰、道具挂饰、领结纹饰等）的颜色应恢复至与参考板中间的原始立绘完全一致的配色，保留「{ACCENT_COLOR}」或「{DOMINANT_COLOR}」100%饱和色彩并在当前基础上提亮30%，与灰度主体形成强烈视觉焦点，除此之外角色全身不得出现任何其他颜色；背景与明暗对比：背景为统一深黑色暗调（RGB 8-15），可带有极微弱的工业噪点纹理，但不得出现任何具体场景、第二种颜色、渐变过渡或暗角；角色与背景形成极端强烈的明暗对比，角色外轮廓清晰锐利，高光区域可接近纯白；文字处理：采用超大号且必须水平排版的做旧印刷体英文{NAME_TOP}和{NAME_BOTTOM}，以双角配对方式放置，文字必须贴近所在角落的边缘位置，严禁旋转、斜切、弧排或透视变形；{NAME_TOP}文字组为三者中最大最宽的文字层，宽度接近画面左右边缘，高度约达画面的四分之一强；{NAME_BOTTOM}文字组明显小于{NAME_TOP}，约为{NAME_TOP}的五到六成大小；{NAME_TOP}位于角色后方作为背景层，{NAME_BOTTOM}压于角色前方作为前景层；文字必须与角色产生重叠和前后遮挡关系，严禁将文字缩到空白角落或边缘空旷处；文字可压住肩、发、手臂等局部，但不得遮挡脸部主体；字体不得横向或纵向拉伸以填满画面，保持正常字形比例；文字不透明度100%、完全实心，不可透过文字看到后方角色；{NAME_TOP}和{NAME_BOTTOM}的文字颜色强制使用比{DOMINANT_COLOR}亮一阶的高饱和颜色，100%不透明度实心字体，不得被去饱和处理影响，不得使用其他颜色',
      promptTemplateEn: 'This is an already-completed zero-fate image. You only need to perform an image-editing-style color transformation on it. It is ABSOLUTELY FORBIDDEN to move, scale, rotate, crop, or regenerate any element in the composition. The character\'s body silhouette, position, size, angle, as well as the text\'s letterforms, kerning, leading, distance from edges, and degree of crop — are ALL LOCKED to exactly what appears in the input image. Not a single pixel of offset is acceptable. The ONLY aspects you may modify are: color grading, rendering style, material expression. The character\'s body silhouette, position, size, angle, as well as the text\'s letterforms, kerning, leading, distance from edges, and degree of crop — are ALL LOCKED to exactly what appears in the input image. Not a single pixel of offset is acceptable. Specific rendering style: transform the dark silhouette aesthetic of the zero-fate image into extreme desaturated grayscale with high-saturation accent colors. You receive a reference board with: LEFT — the zero-fate image (editing base only); MIDDLE — the original character art (ONLY for extracting clothing colors, patterns, and accessory details — strictly forbid copying its pose or composition); RIGHT — style sample (ONLY for color mood and typography reference). Re-emphasize: except for color, every element in the image must be pixel-for-pixel identical to the input image (left side, zero-fate). {CHARACTER_TRAITS} Art style must match the input image (zero-fate). Color treatment — extreme desaturated grayscale + high-saturation accents: the entire character is thoroughly desaturated, retaining only a very small number of designated accent colors. Note: text color is NOT affected by desaturation and must retain high saturation. The clothing structure must not be altered; only color may be changed to a unified black/white/gray with a full grayscale range from near-white (brightness 230-255) to near-black (brightness 10-30), using strong blocky light/shadow modeling and clean contour line art to express three-dimensional layers, fabric folds, and hair texture. Character skin is light-gray to mid-gray (brightness 100-180, fully desaturated). Iris retains {DOMINANT_COLOR} at 80% saturation with a visible specular highlight. Designated accent areas (cape, nails, gloves, lips, heart/star-shaped signature accessories, hair ornaments, prop pendants, bow-tie patterns etc.) should have their colors restored to exactly match the original character art in the middle of the reference board, retaining {ACCENT_COLOR} or {DOMINANT_COLOR} at 100% saturation and brightened by an additional 30%, creating a strong visual focus against the grayscale figure. No other colors anywhere on the character. Background: uniform deep-black dark tone (RGB 8-15), may have extremely faint industrial noise texture but no specific scene, second color, gradient transition, or vignette. Extreme light/dark contrast — silhouette is sharp, highlights may approach pure white. Text: oversized horizontal distressed letterpress English {NAME_TOP} and {NAME_BOTTOM}, placed in two-corner pair, text MUST cling close to corner edges. Forbidden to rotate, skew, arc or perspective-deform. {NAME_TOP} is the largest and widest text block spanning close to edges, roughly one-quarter of the frame height. {NAME_BOTTOM} noticeably smaller, ~50-60% of {NAME_TOP}. {NAME_TOP} behind character (background layer), {NAME_BOTTOM} in front (foreground layer). Text MUST overlap the character — forbidden to shrink into empty corners. May press on shoulders, hair, arms but not face. Must not stretch/compress to fill — keep normal letterform proportions. Text opacity 100%, completely solid. Both colors must use one step brighter than {DOMINANT_COLOR}, high saturation, 100% opacity solid type — must not be affected by the desaturation treatment. Must not use any other color.',
  },
  {
    id: 3,
    label: '六命 · 全彩 / 肤色高光',
    ...
    promptTemplate:
      '这是零命成图（左侧），你需要对其做风格变换。右侧小图为原始角色立绘，仅用于提取服装配色——严禁参考其姿势、构图或人物位置。绝对禁止移动、缩放、旋转、裁切或重新生成左侧画面中的任何元素。唯一可改：色彩、渲染、材质、服装删减（仅六命）。角色身体的轮廓、位置、大小、角度、文字的字形、字距、行距、与边缘的距离、被裁切的程度，全部锁定为输入图片中的样子，不可有任何像素级偏移。具体渲染风格：将零命成图的暗色剪影风格变换为全彩高饱和赛璐珞。画幅比例必须保持与输入图片完全一致的3:2横向构图，严禁改变画幅尺寸或裁剪范围。你收到的参考板左侧为零命成图（仅用于编辑基础），中间为原始角色立绘（仅用于提取服装配色、花纹、配饰细节，严禁复制其角色姿势或构图），右侧为风格样张（仅用于影画色彩排版参考）。再次强调：除色彩外，画面中的一切元素必须与输入图片（左侧零命成图）逐像素完全一致。画风与输入图片（零命成图）保持完全一致；色彩与上色——全彩高饱和赛璐珞：角色进行完整全彩高饱和赛璐珞上色，色彩鲜艳饱满、明亮夺目，严禁暗淡、灰蒙、去饱和或黑白灰处理；真实自然的肤色质感（暖调蜜桃肤色，带有自然红晕与高光）；以「{DOMINANT_COLOR}」为主色调贯穿角色服装、头发、配饰，配合「{ACCENT_COLOR}」作为辉光、高光边缘、点缀色与氛围色提高整体色彩冲击力；肌肤展现（六命专属核心要求，优先级最高）：在保持角色姿势、位置、身体轮廓和面部身份像素级不变的前提下，仅通过对服装部件的大幅删减来增加大幅肌肤露出——必须执行以下至少4项处理：去除外套/披风、去除外层遮挡/护甲、大幅缩短裙摆至大腿以上、大幅缩短袖长至肩部附近、降低领口露出锁骨与肩颈、开放整个背部、开放腰腹、减少腿部布料覆盖至短裤/热裤程度。严禁因删减服装而移动角色的任何身体部位、改变姿势或调整构图。删减仅作用于服装本身，保留的服装必须保持原三视图的版型与材质。背景——纯白无杂质：纯白色（#FFFFFF）铺满整个画面背景区域，无任何渐变、噪点、纹理、阴影或暗角；文字处理：采用超大号且必须水平排版的做旧印刷体英文{NAME_TOP}和{NAME_BOTTOM}，以双角配对方式放置，文字必须贴近所在角落的边缘位置，严禁旋转、斜切、弧排或透视变形；{NAME_TOP}文字组为三者中最大最宽的文字层，宽度接近画面左右边缘，高度约达画面的四分之一强；{NAME_BOTTOM}文字组明显小于{NAME_TOP}，约为{NAME_TOP}的五到六成大小；{NAME_TOP}位于角色后方作为背景层，{NAME_BOTTOM}压于角色前方作为前景层；文字必须与角色产生重叠和前后遮挡关系，严禁将文字缩到空白角落或边缘空旷处；文字可压住肩、发、手臂等局部，但不得遮挡脸部主体；字体不得横向或纵向拉伸以填满画面，保持正常字形比例；文字不透明度100%、完全实心，不可透过文字看到后方角色；{NAME_TOP}和{NAME_BOTTOM}的文字颜色强制使用比{DOMINANT_COLOR}亮一阶的高饱和颜色，两者均不得使用其他颜色。角色动态：{ACTION_POSE}；质感与细节：强烈明暗对比，做旧噪点，斜切几何色块，工业贴纸UI质感，网点纸/半调纹理（halftone dot pattern），印刷做旧质感，扫描线；角色主体脸部清晰，细节高精度，整体呈现绝区零官方Mindscape Cinema影画的明亮全彩印刷品美学。',
      promptTemplateEn: 'The left side is the zero-fate image — you MUST transform its style. The small image on the right is the original character art, ONLY for extracting clothing colors — strictly forbid referencing its pose, composition or character position. It is ABSOLUTELY FORBIDDEN to move, scale, rotate, crop, or regenerate any element in the left image. The ONLY aspects you may modify are: color grading, rendering style, material expression. The character\'s body silhouette, position, size, angle, as well as the text\'s letterforms, kerning, leading, distance from edges, and degree of crop — are ALL LOCKED to exactly what appears in the input image. Not a single pixel of offset is acceptable. Specific rendering style: transform the dark silhouette aesthetic of the zero-fate image into full-color high-saturation cel-shading. The aspect ratio must remain exactly 3:2 landscape — strictly forbid changing the frame dimensions or crop area. You receive a reference board with: LEFT — the zero-fate image (editing base only); MIDDLE — the original character art (ONLY for extracting clothing colors, patterns, and accessory details — strictly forbid copying its pose or composition); RIGHT — style sample (ONLY for color mood and typography reference). Re-emphasize: except for color, every element in the image must be pixel-for-pixel identical to the input image (left side, zero-fate). {CHARACTER_TRAITS} Art style must match the input image (zero-fate). Color & rendering — full-color high-saturation cel-shading: the character receives complete full-color high-saturation cel-shading, colors vivid, saturated, bright and dazzling. Strictly forbid dull, gray, desaturated or black-and-white treatment. Natural skin texture (warm peach skin tone with natural blush and specular highlights). Use {DOMINANT_COLOR} as the dominant color throughout the character\'s clothing, hair and accessories, with {ACCENT_COLOR} as glow, rim light, accent and atmospheric color for maximum color impact. Skin Exposure (SIX-FATE EXCLUSIVE, HIGHEST PRIORITY): while keeping the character\'s pose, position, body silhouette and facial identity locked at the pixel level, increase skin exposure solely through drastic reduction of clothing parts — must execute at least 4 of the following: remove jacket/cape, remove outer armor/covering, drastically shorten skirt to above thigh, drastically shorten sleeves to near shoulder, lower neckline to expose collarbone and shoulders, open the entire back, expose waist/abs, reduce leg coverage to shorts/hotpants level. Forbidden to move any body part, change pose, or adjust composition as a result of clothing reduction. Reduction only acts on the clothing itself. Retained clothing must keep the original cut and material. Background — pure white, no impurities: pure white (#FFFFFF) filling the entire background area. No gradient, noise, texture, shadow, or vignette. Text: oversized horizontal distressed letterpress English {NAME_TOP} and {NAME_BOTTOM}, placed in two-corner pair, text MUST cling close to corner edges. Forbidden to rotate, skew, arc or perspective-deform. {NAME_TOP} is the largest and widest text block spanning close to edges, roughly one-quarter of the frame height. {NAME_BOTTOM} noticeably smaller, ~50-60% of {NAME_TOP}. {NAME_TOP} behind character (background layer), {NAME_BOTTOM} in front (foreground layer). Text MUST overlap the character — forbidden to shrink into empty corners. May press on shoulders, hair, arms but not face. Must not stretch/compress to fill — keep normal letterform proportions. Text opacity 100%, completely solid. Both colors must use one step brighter than {DOMINANT_COLOR}, high saturation. Must not use any other color. Character dynamic: {ACTION_POSE}. Texture & details: strong light/dark contrast, aged noise, diagonal geometric blocks, industrial sticker UI texture, halftone dot pattern, aged print texture, scan lines. Character\'s face clear, high-precision detail. Overall: ZZZ Official Mindscape Cinema bright full-color print aesthetic.',
  },
];
