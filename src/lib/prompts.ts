...
export const YINGHUA_STYLES: YinghuaStyle[] = [
  {
    ...
    promptTemplate:
      '参考图包含原始角色立绘与风格样张：原始角色立绘用于身份、面部、发型、服饰与配色参考；样张仅用于绝区零影画的整体构图方向、色彩关系与角落文字排版参考，严禁借用样张中的人物外形和名字。绝区零官方Mindscape Cinema影画风格，暗色剪影印刷美学（Dark Silhouette Print Style）：基于原始角色立绘生成基准图{CHARACTER_TRAITS}；人物构图允许明显偏左或偏右，严禁正面平视、证件照式大头、对称站像；允许头顶、肩部、手臂、发尾等局部出框，人物主体大面积裁切，优先冲击感而不是完整呈现；角色主体和文字共占约占画面80%~85%；动作设计具有艺术感、展现角色个性特点；画风与原三视图保持完全一致。角色主体应以接近纯黑的深黑块面为主，形成明确的黑版主体（不是背景色）；外轮廓必须硬朗清晰、边缘分明、黑白块面关系明确。主体整体更接近平面压印效果，而不是明显三维光影建模；主体的任何局部明暗必须是配色占比的直接结果，不得出现因光源方向、模拟打光或阴影投射而产生的额外亮面或暗面；只保留极弱的明暗差异与少量边缘轮廓变化，用于提示头发、服装结构、布料褶皱、配饰轮廓和手部姿态。角色内部色彩控制：脸部与身体肤色保持接近纯黑的深黑色，仅允许约5%的极微量{DOMINANT_COLOR}或{ACCENT_COLOR}微弱渗入，严禁脸部发亮或面部建模高光；角色内部相对亮色应集中在服饰、衣物配件、饰品、发饰、头发边缘等非皮肤部位，这些部位可作为点缀区域承载更多{DOMINANT_COLOR}或{ACCENT_COLOR}的深暗色，约15%~25%；主体与背景必须有明显分离，不允许主体与背景同色化。服装本体、花纹、布料印花、袖口纹样、腰带图案都应压在深暗色域内，不应出现醒目亮色；花纹仅允许以低对比度暗纹、压印感或几乎融入布料的深色纹样存在，不得形成高可读性亮花纹。高光只限极少量眼部微光、极细外轮廓描线和少量发丝边缘，不允许大面积衣料高光、亮纹样、亮带子、被提亮的发饰，或明显发光的服饰花纹。主体应更深、更压，但必须与高饱和背景形成明确分离；只允许极少量边缘被背景色轻微染边，主体大面积块面绝不能变成背景色本身。背景必须是高饱和亮色纯色平涂，优先使用{DOMINANT_COLOR}或{ACCENT_COLOR}作为大面积底色；不要贴纸、条码、标签、图标、网点纸、胶带、扫描线或任何工业装饰。文字处理：采用超大号但正常比例、且必须水平排版的做旧印刷体英文{NAME_TOP}和{NAME_BOTTOM}，以双角配对方式放置（左上+左下/左上+右下/右上+左下/右上+右下四选一），baseline 与画面上下边平行，严禁旋转、斜切、弧排、透视变形以及横向/纵向拉伸压缩；单组文字应当占据画面高度的1/3、宽的65%~85%，但不得压缩或拉伸字形，可通过适度扩大字符间隙来适配版面宽度；优先单侧或对角大字压版，贴近边缘、接近满幅块面，但不是整条上/下边缘铺满；可大面积占据角落块面，允许未放字的其余角落自然留白；文字可明显压住肩、发、手臂等主体局部，但不得遮挡脸部主体；{NAME_TOP}文字颜色使用比背景更深一阶的近黑版印刷色，而不是亮色文字块，保留轻微磨损与印刷质感，使文字更像压印在海报上的黑版字层。{NAME_BOTTOM}文字颜色使用比背景更亮一阶的印刷色，而不是暗色文字块，保留轻微磨损与印刷质感，使文字更像压印在海报上的黑版字层。角色动态：{ACTION_POSE}；整体效果：高对比、强设计感、深黑压印主体、主题色背景、水平超大角落压版文字、强烈留白与斜向冲击感。',
  },
  {
    id: 2,
    label: '三命 · 暗色背景 / 亮色主角',
  ...
  },
  {
    ...
    promptTemplate:
      '参考图为左右拼合双图：左侧为零命成图（姿势构图与文字位置参考），右侧为原始角色立绘（服饰配色参考）。绝区零影画风格，依照左侧零命参考图保持完全相同的角色姿势、构图比例、取景范围与文字位置，严禁任何位移或缩放{CHARACTER_TRAITS}；角色充满画面（与零命一致）；画风与原三视图保持完全一致；色彩处理——极端去饱和灰度主调 + 高饱和点缀色：角色整体进行彻底去饱和处理，仅保留极少数指定点缀色；服装本体（上衣、裙子、裤子、外套等）与角色肌肤统一为黑白灰三色，建立从近白（亮度230-255）到近黑（亮度10-30）的完整灰阶，通过强烈的块面光影明暗（高光→中间调→阴影）与清晰外轮廓线稿体现立体层次、布料褶皱与发丝质感；角色肌肤为浅灰至中灰色调（亮度100-180，完全去饱和）；眼睛虹膜保留「{DOMINANT_COLOR}」（80%色彩饱和度），并带有明显的高光反射点；指定点缀部位（包括但不限于：指甲、唇舌、心形/星形等标志性配饰、发饰、道具挂饰、领结纹饰等）保留「{ACCENT_COLOR}」或「{DOMINANT_COLOR}」高饱和色彩（90%饱和度），与灰度主体形成强烈视觉焦点，除此之外角色全身不得出现任何其他颜色；背景与明暗对比：背景为统一深黑色暗调（RGB 8-15），可带有极微弱的工业噪点纹理，但不得出现任何具体场景、第二种颜色、渐变过渡或暗角；角色与背景形成极端强烈的明暗对比，角色外轮廓清晰锐利，高光区域可接近纯白；文字处理：正常比例做旧印刷体英文「{NAME_TOP}」和「{NAME_BOTTOM}」以双角配对方式放置（左上+左下/左上+右下/右上+左下/右上+右下四选一），不拉伸不压缩，不铺满边缘，采用高饱和「{ACCENT_COLOR}」做旧色，边缘带有粗糙磨损、噪点、印刷错位与墨点飞溅质感；文字可与角色叠压，允许被角色部分遮挡；质感强化：工业贴纸UI质感，斜切几何色块，网点纸/半调纹理（halftone dot pattern），印刷做旧质感，扫描线，角色主体脸部清晰可辨，整体呈现绝区零官方Mindscape Cinema影画的印刷品美学。',
  },
];
