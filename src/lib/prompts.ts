...
export const YINGHUA_STYLES: YinghuaStyle[] = [
  ...
  },
  {
    ...
    promptTemplate:
      '参考图为左右拼合双图：左侧为零命成图（姿势构图与文字位置参考），右侧为原始角色立绘（服饰配色参考）。绝区零影画风格，依照左侧零命参考图保持完全相同的角色姿势、构图比例、取景范围与文字位置，严禁任何位移或缩放{CHARACTER_TRAITS}；角色和文字必须像素级对齐左侧零命图片的位置，任何偏离都会导致废图，包括但不限于：角色朝向、头部倾斜角度、身体偏移量、文字的字间距、行距、与边缘的距离、被裁切的程度都必须与左侧零命图片完全一致；角色充满画面（与零命一致）；画风与原三视图保持完全一致；色彩与上色——全彩高饱和赛璐珞：角色进行完整全彩高饱和赛璐珞上色，色彩鲜艳饱满、明亮夺目，严禁暗淡、灰蒙、去饱和或黑白灰处理；真实自然的肤色质感（暖调蜜桃肤色，带有自然红晕与高光）；以「{DOMINANT_COLOR}」为主色调贯穿角色服装、头发、配饰，配合「{ACCENT_COLOR}」作为辉光、高光边缘、点缀色与氛围色提高整体色彩冲击力；肌肤展现：在内容合规、非露骨的前提下大幅增加肌肤展现（露肩、露臂、露背、露腰、露腿等），可对原角色服装进行破损、撕裂、减少布料、去除外层遮挡、缩短裙摆/袖长、降低领口等处理，仅保留原角色服饰的核心辨识元素与标志性配色；背景——纯白无杂质：纯白色（#FFFFFF）铺满整个画面背景区域，无任何渐变、噪点、纹理、阴影或暗角；文字处理：采用超大号但正常比例、且必须水平排版的做旧印刷体英文{NAME_TOP}和{NAME_BOTTOM}，以双角配对方式放置（左上+左下/左上+右下/右上+左下/右上+右下四选一），baseline 与画面上下边平行，严禁旋转、斜切、弧排、透视变形以及横向/纵向拉伸压缩；{NAME_TOP}文字组为三者中最大、最宽的文字层，宽度应接近画面左右边缘，约占画面高度的35%~40%；{NAME_BOTTOM}文字组明显小于{NAME_TOP}，约占{NAME_TOP}宽度的77%~98%、高度的70%~84%。两者均不得压缩或拉伸字形，可通过适度扩大字符间隙来适配版面宽度；优先单侧或对角大字压版，{NAME_TOP}位于角色后方作为背景层，{NAME_BOTTOM}压于角色前方作为前景层，贴近边缘、接近满幅块面，但不是整条上/下边缘铺满；可大面积占据角落块面，允许未放字的其余角落自然留白；文字可明显压住肩、发、手臂等主体局部，但不得遮挡脸部主体；{NAME_TOP}和{NAME_BOTTOM}的文字颜色提取左侧零命参考图中{NAME_BOTTOM}已有的文字颜色并保持一致，不得自行更改色彩，保留轻微磨损与印刷质感，使文字更像压印在海报上的字层。角色动态：{ACTION_POSE}；质感与细节：强烈明暗对比，做旧噪点，斜切几何色块，工业贴纸UI质感，网点纸/半调纹理（halftone dot pattern），印刷做旧质感，扫描线；角色主体脸部清晰，细节高精度，整体呈现绝区零官方Mindscape Cinema影画的明亮全彩印刷品美学。',
  },
];
